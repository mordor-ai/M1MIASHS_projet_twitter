...
def get_parameters(graph: Graph, directed=False, unconn=True):
    ...
    param_dict['centralization'] = len (graph.vs) / (len (graph.vs) - 2) * (
            max (map (lambda x: len (x), graph.neighborhood ( ))) / (len (graph.vs) - 1) - graph.density ( ))
    param_dict['heterogeneity'] = cmath.sqrt (variance (map (lambda x: len (x), graph.neighborhood ( )))).real / mean (
        map (lambda x: len (x), graph.neighborhood ( )))
    param_dict['multiple_edge_node_pair_number'] = len (
        list (set ([sorted ((x['from'], x['to'])) for x in graph.es if graph.is_multiple (x)])))
...
def radiality_centrality(graph):
    ...
    score_dict = {}
    for v in graph.vs:
        score_dict[v] = sum (map (lambda x: graph.shortest_paths (source=v.index, target=x.index)[0][0] == float (
            'Inf') and diameter + 1 or diameter + 1 - graph.shortest_paths (source=v.index, target=x.index)[0][0] / (
                                                    size - 1), graph.vs))
...
def centroid_centrality(graph: Graph):
    ...
    for v1 in xrange (0, len (graph.vs)):
        for v2 in xrange (0, len (graph.vs)):
            fm[v1][v2] = len (filter (lambda x: x[0] < x[1], zip (pathm[v1], pathm[v2]))) - len (
...
def eccentricity_centrality(graph):
    ...
    return sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))], key=lambda x: x[1],
                   reverse=True)
...
#	degree centrality
def degree_centrality(graph: Graph, mode_in_out_all):
    score_list = graph.degree (mode=mode_in_out_all)
    return sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))], key=lambda x: x[1],
                   reverse=True)


#	betweenness centrality
def betweenness_centrality(graph: Graph, directed: bool = False):
...
def closeness_centrality(graph: Graph, mode_in_out_all):
    score_list = graph.closeness (mode=mode_in_out_all)
    return sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))], key=lambda x: x[1],
                   reverse=True)
...
#	pagerank centrality
def pagerank_centrality(graph: Graph):
...
def analyze_centrality(graph):
    ...
    centrality_dict['degree'] = sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))],
    ...
    score_list = graph.betweenness ( )
    centrality_dict['betweenness'] = sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))],
                                             key=lambda x: x[1], reverse=True)
    print ('Done!')
    print
    ...
    centrality_dict['closeness'] = sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))],
                                           key=lambda x: x[1], reverse=True)
    print ('Done!')
    print
    ...
    score_list = graph.evcent ( )
    centrality_dict['eigenvector'] = sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))],
                                             key=lambda x: x[1], reverse=True)
    print ('Done!')
    print
    ...
    score_list = graph.pagerank ( )
    centrality_dict['pagerank'] = sorted ([(graph.vs[i], score_list[i]) for i in xrange (0, len (score_list))],
                                          key=lambda x: x[1], reverse=True)
    print ("Done!")
